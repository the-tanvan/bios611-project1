...
```{r}
fit = glm(Male~Weight+Height,data=analysis,family=binomial())
prediction=(predict(fit,type="response",analysis)>0.5)
results=as.data.frame(cbind(Truth=analysis$Male,Prediction=prediction)) %>% mutate(correct=(Truth==Prediction))
...
fit2 = glm(Male~Weight+Height,data=filter,family=binomial())
prediction2=(predict(fit2,type="response",filter)>0.5)
results2=as.data.frame(cbind(Truth=filter$Male,Prediction=prediction2)) %>% mutate(correct=(Truth==Prediction))
